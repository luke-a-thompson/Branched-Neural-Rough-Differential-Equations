...
def _hidden_geometry(
    ...
    if isinstance(data_geometry, georax.SO):
        if int(state_param_dim) not in (6, 9):
            raise ValueError(
                "SO(3) initializes from a 6D representation or a direct 3x3 state, "
                f"so initial_state_param_dim must be 6 or 9; got {state_param_dim}."
            )
    elif isinstance(data_geometry, georax.SPD):
        expected = data_geometry.coordinate_shape[0]
        if int(state_param_dim) != expected:
            raise ValueError(
                f"SPD({data_geometry.n}) requires initial_state_param_dim={expected}; "
                f"got {state_param_dim}."
            )
    return (
        data_geometry,
        data_geometry.state_shape,
        data_geometry.coordinate_shape[0],
    )
...
class BNRDE(eqx.Module):
    ...
    control_has_time_channel: bool = eqx.field(static=True)
    ...
    def __init__(
        ...
        *,
        ...
        control_has_time_channel: bool = False,
        ...
        geometry, state_shape, frame_dim = _hidden_geometry(
            data_geometry,
            hidden_state_mode,
        ...
        if hidden_state_mode == HiddenStateMode.PROBLEM_MANIFOLD:
            expected_output_dim = (
                frame_dim
                if isinstance(data_geometry, georax.SPD)
                else math.prod(state_shape)
            )
        ...
        self.control_has_time_channel = bool(control_has_time_channel)
    ...
    def _initial_hidden(self, x0: jax.Array) -> jax.Array:
        ...
        if self.hidden_state_mode == HiddenStateMode.EUCLIDEAN:
            return raw
        if isinstance(self.data_geometry, georax.SO):
            return project_to_manifold(self.data_geometry, raw)
        if isinstance(self.data_geometry, georax.SPD):
            sym = spd_unvech(raw)
        ...
        raise ValueError(
            f"Could not initialize BNRDE state for {type(self.data_geometry).__name__}."
        )
    ...
    def _apply_readout(self, hidden_states: jax.Array) -> jax.Array:
        ...
        def apply_single(y: jax.Array) -> jax.Array:
            ...
            return project_to_manifold(self.data_geometry, activation)
    ...
    def _solve_from_values(
        ...
        correction = (
            ito_correction(
                ...
                has_time_channel=self.control_has_time_channel,
            )
